...
function parse(formData: FormData) {
  return designSchema.safeParse({
    ...
    dieId: formData.get("dieId"),
    plateId: formData.get("plateId"),
    dieStatus: formData.get("dieStatus"),
    plateStatus: formData.get("plateStatus"),
    ...
    const created = await db.transaction(async (tx) => {
      ...
      const row = await auditedInsert(
        ...
        design,
        {
          ...
          dieId: orNull(v.dieId),
          plateId: orNull(v.plateId),
          dieStatus: v.dieStatus,
